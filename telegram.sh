...
MESSAGE=""
if [[ ${NOTIFY_WHAT} == "SERVICE" ]]; then
	MESSAGE+="${EMOJI} ***${NOTIFY_SERVICEDESC}*** ${EMOJI}%0A%0A"
	MESSAGE+="***Check_MK: ${NOTIFY_HOSTNAME}/${NOTIFY_SERVICEDESC}***%0A"
	MESSAGE+="\`\`\`%0A"
	MESSAGE+="Host:     ${NOTIFY_HOSTNAME}%0A"
	MESSAGE+="Address:  ${NOTIFY_HOST_ADDRESS_4}%0A"
	MESSAGE+="Service:  ${NOTIFY_SERVICEDESC}%0A"
	MESSAGE+="Event:    ${NOTIFY_PREVIOUSSERVICEHARDSHORTSTATE} -> ${NOTIFY_SERVICESHORTSTATE}%0A"
	MESSAGE+="Output:   ${NOTIFY_SERVICEOUTPUT}%0A"
	MESSAGE+="\`\`\`"
else
	MESSAGE+="${EMOJI} ***${NOTIFY_HOSTNAME}*** ${EMOJI}%0A%0A"
	MESSAGE+="***Check_MK: ${NOTIFY_HOSTNAME}***%0A"
	MESSAGE+="\`\`\`%0A"
	MESSAGE+="Host:     ${NOTIFY_HOSTNAME}%0A"
	MESSAGE+="Address:  ${NOTIFY_HOST_ADDRESS_4}%0A"
	MESSAGE+="Event:    ${NOTIFY_PREVIOUSHOSTHARDSHORTSTATE} -> ${NOTIFY_HOSTSHORTSTATE}%0A"
	MESSAGE+="Output:   ${NOTIFY_HOSTOUTPUT}%0A"
	MESSAGE+="\`\`\`"
fi
